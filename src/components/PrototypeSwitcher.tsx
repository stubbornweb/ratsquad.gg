...
export function PrototypeSwitcher({ variants, current, children }: Props) {
  ...
  return (
    <div className="fixed bottom-5 left-1/2 z-[500] flex -translate-x-1/2 items-stretch border-2 border-white bg-black font-mono text-[11px] tracking-widest text-white uppercase shadow-[0_8px_32px_rgba(0,0,0,0.8)]">
      <button
        onClick={() => go(-1)}
      ...
      <div className="flex min-w-[280px] items-center justify-center border-x-2 border-white px-4 py-3">
      ...
      {children ? (
        <div className="flex items-stretch border-l-2 border-white">{children}</div>
      ) : null}
    </div>
  );
}
