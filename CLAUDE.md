...
RATS is a competitive EU clan in **Squad**, the hardcore tactical shooter. This site is the clan's public face and recruitment funnel — every path ends at the Discord invite.

**UI copy is Ukrainian** (`<html lang="uk">` in `src/app/layout.tsx`). Code, comments, commit messages and this file are English. New user-facing text goes in Ukrainian, matching the terse uppercase register already in `src/data/`.

## Tech stack

- **Next.js 16.1.7**, App Router, React 19.2.4, TypeScript 5.9.3
- **Tailwind CSS v4** — CSS-first config in `src/app/globals.css`; there is no `tailwind.config.ts`
- **Framer Motion 12.38** for animation, **Lenis** for smooth scroll, **Lucide** for icons
- `clsx` + `tailwind-merge` behind the `cn()` helper in `src/lib/utils.ts`
- Deployed on **Vercel**, with Analytics and Speed Insights enabled

## Where things live

- `src/app/` — routes, layouts, `globals.css`, `sitemap.ts`, `robots.ts`. Two routes: `/` and `/roster`.
- `src/components/sections/` — the homepage sections (Hero, About, RosterPreview, Join, FAQ, Discord).
- `src/components/ui/` — small presentational primitives (glitch text, counters, dividers).
- `src/components/` (root) — shared chrome: Navbar, Footer, page transitions, loading screen.
- `src/data/` — static content: roster fallback, FAQ, about pillars, join requirements.
...
- **Design tokens live in `src/app/globals.css`** (`:root`, lines 17-51) and are the single source of truth. Read them; never restate their values elsewhere. Fonts are loaded in `src/app/layout.tsx` via `next/font/google`.
...
GitHub Issues on `stubbornweb/ratsquad.gg`, via the `gh` CLI. See `docs/agents/issue-tracker.md`.
...
- Treat Vercel Functions as stateless and ephemeral — no durable filesystem, no background daemons
- Store secrets in Vercel env variables, never in git or `NEXT_PUBLIC_*`
- Sync env and project settings with `vercel env pull` / `vercel pull` for local parity
- If Deployment Protection is enabled, use a bypass secret to reach preview deployments
